...
import { FullLogo, LogoSymbol } from './components/Branding';
...
const App: React.FC = () => {
  ...
  // Timeline UI
  const [timelineHeight, setTimelineHeight] = useState('320px');
  ...
  useEffect(() => {
    ...
    // Load local data (fallback/initial)
  ...
  useEffect(() => {
    if (isSyncEnabled) {
      const unsubscribe = subscribeToCollections({
        ...
        onLogs: (l) => setLogs(l),
        onObservations: (o) => setObservations(o),
        onOffDays: (d) => setOffDays(d),
        onConfig: (c) => {
            if (c) setAppConfig(prev => ({ ...prev, ...c }));
        }
      });
  ...
  const persistData = (
    ...
    if (isSyncEnabled) {
        // Syncing handled by individual actions ideally, but for now we push full state if needed
        // Or better, we should use syncData with specific actions. 
        // For simplicity in this restoration, we are using the bulk save logic or assuming syncData handles it.
        // We will adapt the persistData to use syncData for full updates if supported, 
        // but `syncData` in firebaseService takes actions. 
        // Since we are refactoring, let's create a 'full' sync action for simplicity here.
        syncData([{ 
            type: 'full', 
            action: 'overwrite', 
            data: { tasks: newTasks, logs: newLogs, observations: newObs, offDays: newOffDays, appConfig: newConfig } 
        }]);
    }
  };
  ...
  const addUpdateToTask = (id: string, content: string, attachments?: TaskAttachment[], highlightColor?: string) => {
    ...
    const newUpdate: TaskUpdate = { id: updateId, timestamp, content, attachments, highlightColor };
    
    const updated = tasks.map(t => t.id === id ? { ...t, updates: [...t.updates, newUpdate] } : t);
    ...
    if (selectedTask?.id === id) {
        setSelectedTask(prev => prev ? { ...prev, updates: [...prev.updates, newUpdate] } : null);
    }
  };
  ...
  const handleEditUpdate = (taskId: string, updateId: string, content: string, timestamp?: string, highlightColor?: string | null) => {
    ...
    const newLogs = logs.map(l => {
      if (l.taskId === taskId) {
        ...
        // Simplistic check to link log to update. In a real app, updates should link to log IDs.
    ...
    if (selectedTask?.id === taskId) {
        // Update selected task state locally to reflect in modal immediately
  ...
  const filteredTasks = useMemo(() => {
    ...
    const base = tasks.filter(t => (t.description || '').toLowerCase().includes(q) || t.displayId.toLowerCase().includes(q) || (t.title || '').toLowerCase().includes(q));
  ...
  const getStatusColorMini = (s: string) => {
    const custom = appConfig.itemColors?.[s];
    if (custom) return `bg-[${custom}] text-white`;
          ...
          <div className="space-y-6 animate-fade-in">
             ...
             <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                    ...
                    {statusSummary.map(s => (
                        <div key={s.label} className="bg-slate-50 dark:bg-slate-700/50 border border-slate-100 dark:border-slate-700 p-4 rounded-xl flex flex-col justify-between hover:bg-white dark:hover:bg-slate-700 hover:border-indigo-100 dark:hover:border-indigo-900 transition-all group">
                            ...
                            <div className="flex items-end justify-between mt-2">
                                ...
                                <div className="p-1 bg-white dark:bg-slate-800 rounded border border-slate-100 dark:border-slate-600 shadow-xs">
                                    <div className="w-1.5 h-1.5 rounded-full bg-slate-300 dark:bg-slate-500 group-hover:bg-indigo-400" />
                                </div>
                            </div>
                        </div>
                    ))}
        ...
        return (
          <div className="h-full flex flex-col space-y-6 animate-fade-in">
                    ...
                    <div key={d} className={`min-w-[280px] w-[280px] p-4 rounded-2xl border flex flex-col transition-all h-full ${d === todayStr ? 'bg-indigo-50 dark:bg-indigo-900/20 border-indigo-200 dark:border-indigo-800 ring-2 ring-indigo-100 dark:ring-indigo-900 shadow-md z-10' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 shadow-sm'}`}>
                        ...
                        <div className="flex-1 overflow-y-auto space-y-2 pr-1 custom-scrollbar min-h-0">
                            {weekTasks[d]?.length ? weekTasks[d].map(t => {
                                const isDone = t.status === Status.DONE || t.status === Status.ARCHIVED;
                                const priorityColor = appConfig.itemColors?.[t.priority] || 
                                    (t.priority === Priority.HIGH ? '#ef4444' : 
                                     t.priority === Priority.MEDIUM ? '#f59e0b' : 
                                     t.priority === Priority.LOW ? '#10b981' : '#cbd5e1');

                                return (
                                    <div 
                                      key={t.id} 
                                      onClick={() => setSelectedTask(t)} 
                                      className={`p-3 rounded-xl border transition-all cursor-pointer group flex flex-col gap-2 shadow-sm hover:shadow-md ${
                                          isDone 
                                          ? 'bg-slate-100 dark:bg-slate-900/40 border-slate-200 dark:border-slate-800 opacity-75' 
                                          : 'bg-white dark:bg-slate-900/50 border-slate-200 dark:border-slate-700 hover:border-indigo-400 dark:hover:border-indigo-500'
                                      }`}
                                    >
                                        <div className="flex justify-between items-center">
                                            <span className={`text-[10px] font-mono font-bold tracking-wider ${isDone ? 'text-slate-400' : 'text-slate-500 dark:text-slate-400'}`}>{t.displayId}</span>
                                            <div 
                                                className="w-2.5 h-2.5 rounded-full shadow-sm ring-1 ring-white dark:ring-slate-900"
                                                style={{ backgroundColor: priorityColor }}
                                                title={`Priority: ${t.priority}`}
                                            ></div>
                                        </div>
                                        
                                        <p className={`text-sm font-bold leading-tight line-clamp-3 mb-1 ${
                                            isDone 
                                        ...
                                        <div className="mt-auto pt-1 flex justify-end">
                                            <span className={`text-[9px] font-black px-1.5 py-0.5 rounded uppercase tracking-wider ${getStatusColorMini(t.status)}`}>
                                                {t.status}
                                            </span>
                                        </div>
                                    </div>
             ...
             <div className="flex-1 min-h-0 grid grid-cols-1 xl:grid-cols-3 gap-8">
                <div className="xl:col-span-2 flex flex-col bg-slate-100/50 dark:bg-slate-900/20 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-inner">
                    <div className="bg-white dark:bg-slate-800 p-5 border-b border-slate-200 dark:border-slate-700 flex flex-wrap items-center justify-between gap-4">
                        <div className="flex bg-slate-100 dark:bg-slate-700 p-1 rounded-xl">
                            <button onClick={() => setActiveTaskTab('current')} className={`px-5 py-2 rounded-lg text-sm font-bold transition-all ${activeTaskTab === 'current' ? 'bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}>Active Tasks</button>
                            <button onClick={() => setActiveTaskTab('completed')} className={`px-5 py-2 rounded-lg text-sm font-bold transition-all ${activeTaskTab === 'completed' ? 'bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}>Archive & Done</button>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar bg-slate-50 dark:bg-slate-900">
                ...
                <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-lg overflow-hidden flex flex-col h-full">
                    <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
                        <DailyJournal 
                            ...
                            onEditLog={(logId, taskId, content, date) => {
                                const newLogs = logs.map(l => l.id === logId ? { ...l, taskId, content, date } : l);
                                persistData(tasks, newLogs, observations, offDays);
                            }}
                            onDeleteLog={(logId) => {
                                if (confirm('Delete log?')) persistData(tasks, logs.filter(l => l.id !== logId), observations, offDays);
                            }}
                            ...
                            onOpenTask={(t, content) => {
                                setSelectedTask(t);
                                // could pass content to scroll to specific update
                            }}
                        />
                    </div>
                </div>
      ...
      case ViewMode.SETTINGS:
        return (
          <Settings 
            ...
            observations={observations} 
            offDays={offDays}
            onImportData={(d) => persistData(d.tasks || [], d.logs || [], d.observations || [], d.offDays || [], d.appConfig || appConfig)} 
            onSyncConfigUpdate={c => setIsSyncEnabled(!!c)} 
            isSyncEnabled={isSyncEnabled} 
            appConfig={appConfig} 
            onUpdateConfig={c => { setAppConfig(c); localStorage.setItem('protrack_app_config', JSON.stringify(c)); }} 
  ...
  return (
    <div className={`flex h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans overflow-hidden ${isDarkMode ? 'dark' : ''}`}>
      <aside className={`${isSidebarOpen ? 'w-64' : 'w-20'} bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 transition-all duration-300 flex flex-col z-20 shadow-xl`}>
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
           {[
             ...
             <button key={item.mode} onClick={() => setView(item.mode)} className={`w-full flex items-center gap-3 p-3 rounded-xl transition-all ${view === item.mode ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 font-bold shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>
                <item.icon size={20} />
                {isSidebarOpen && <span>{item.label}</span>}
             </button>
           ))}
           <div className="pt-4 mt-4 border-t dark:border-slate-800">
             <button onClick={() => setView(ViewMode.SETTINGS)} className={`w-full flex items-center gap-3 p-3 rounded-xl transition-all ${view === ViewMode.SETTINGS ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 font-bold shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>
                <SettingsIcon size={20} />
                {isSidebarOpen && <span>Settings</span>}
             </button>
             <button onClick={() => setView(ViewMode.HELP)} className={`w-full flex items-center gap-3 p-3 rounded-xl transition-all ${view === ViewMode.HELP ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 font-bold shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>
                <HelpCircle size={20} />
        ...
        <div className="p-4 border-t dark:border-slate-800">
           <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-2 text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg w-full flex justify-center">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden bg-slate-50 dark:bg-slate-950">
        <div className="h-16 bg-white dark:bg-slate-900 border-b dark:border-slate-800 flex items-center justify-between px-6 shrink-0 z-10 shadow-sm">
           <div className="relative max-w-md w-full">
              <Search size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500" />
              <input type="text" placeholder="Search tasks, logs, projects..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2 bg-slate-50 dark:bg-slate-800 border-none rounded-lg text-sm outline-none dark:text-white dark:placeholder-slate-500 focus:ring-2 focus:ring-indigo-500/20" />
           </div>
           <div className="flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full ${isSyncEnabled ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'bg-slate-300 dark:bg-slate-600'}`}></div>
        ...
        {showNewTaskModal && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
             <form onSubmit={handleCreateTask} className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-fade-in border border-slate-200 dark:border-slate-800">
                <div className="p-5 border-b dark:border-slate-800 flex justify-between items-center bg-indigo-600 dark:bg-indigo-900/50 text-white">
                ...
                <div className="p-6 space-y-4">
                   ...
                   <div className="space-y-1">
                      ...
                      <textarea 
                        ...
                        className="w-full px-3 py-2 text-sm bg-slate-50 dark:bg-slate-800 border dark:border-slate-700 rounded-xl outline-none focus:ring-2 focus:ring-indigo-100 dark:focus:ring-indigo-900 dark:text-white resize-none" 
                      />
                   </div>
                   <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1">
                ...
                <div className="p-4 border-t dark:border-slate-800 bg-slate-50 dark:bg-slate-800 flex justify-end gap-3">
        ...
        {showReportModal && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
             <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden border border-slate-200 dark:border-slate-800 animate-fade-in">
                <div className="p-4 border-b dark:border-slate-800 flex justify-between items-center bg-indigo-600 dark:bg-indigo-900/50 text-white">
                ...
                <div className="flex-1 overflow-y-auto p-6 bg-white dark:bg-slate-900">
                   {isGeneratingReport ? (
                        <div className="flex flex-col items-center justify-center py-12 gap-4">
                            ...
                            <p className="text-slate-500 dark:text-slate-400 animate-pulse">Analyzing task data...</p>
                        </div> 
                    ) : (
                        <div className="prose prose-sm dark:prose-invert max-w-none">
                            {generatedReport.split('\n').map((line, i) => {
                                // Basic markdown parsing for the report
                                if (line.startsWith('## ')) return <h3 key={i} className="text-lg font-bold mt-4 mb-2 text-slate-800 dark:text-white">{line.replace('## ', '')}</h3>;
                                if (line.startsWith('### ')) return <h4 key={i} className="text-md font-bold mt-3 mb-1 text-slate-700 dark:text-slate-200">{line.replace('### ', '')}</h4>;
                                if (line.startsWith('- ') || line.startsWith('* ')) return <li key={i} className="ml-4 text-slate-600 dark:text-slate-300">{line.replace(/[-*] /, '')}</li>;
                                return <p key={i} className="mb-2 text-slate-600 dark:text-slate-300">{line}</p>;
                            })}
                        </div>
                    )}
                </div>
                <div className="p-4 border-t dark:border-slate-800 flex justify-end gap-2 bg-slate-50 dark:bg-slate-800">
                   <button onClick={() => { navigator.clipboard.writeText(generatedReport); alert('Copied!'); }} className="px-4 py-2 text-slate-600 dark:text-slate-400 font-bold rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700">Copy Text</button>
      ...
      <AIChat 
        ...
        observations={observations} 
        appConfig={appConfig} 
        onOpenSettings={() => setView(ViewMode.SETTINGS)} 
      />
    </div>
  );
};
