...
const DailyJournal: React.FC<DailyJournalProps> = ({ tasks, logs, onAddLog, onUpdateTask, offDays = [], onToggleOffDay }) => {
  ...
  const handleAddQuickLog = (e: React.FormEvent) => {
      e.preventDefault();
      if (!quickLog.trim() || !selectedTaskId) return;
      onAddLog({ date: entryDate, taskId: selectedTaskId, content: quickLog.trim() });
      setQuickLog('');
  };
    ...
    <div className="space-y-6 h-full flex flex-col">
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex items-center justify-between">
            ...
            {onToggleOffDay && (
                <button 
                  ...
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-bold transition-colors ${offDays.includes(entryDate) ? 'bg-red-100 text-red-700 hover:bg-red-200' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                >
                  <Ban size={14} />
                  {offDays.includes(entryDate) ? 'Set as Work Day' : 'Mark as Off Day'}
                </button>
            )}
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2 bg-white p-3 rounded-lg border border-slate-200 shadow-sm">
        <div className="flex items-center justify-between">
           ...
           <button onClick={() => setViewRange({ start: getStartOfWeek(new Date()), end: getEndOfWeek(new Date()) })} className="text-xs text-indigo-600 font-medium hover:underline flex items-center gap-1">
             <RotateCcw size={12}/> Reset Week
           </button>
        </div>
        <div className="flex items-center gap-2">
           <input type="date" value={viewRange.start} onChange={(e) => setViewRange(prev => ({ ...prev, start: e.target.value }))} className="w-full text-xs p-1.5 border border-slate-200 rounded bg-slate-50 outline-none focus:border-indigo-500" />
           <span className="text-slate-400">-</span>
           <input type="date" value={viewRange.end} onChange={(e) => setViewRange(prev => ({ ...prev, end: e.target.value }))} className="w-full text-xs p-1.5 border border-slate-200 rounded bg-slate-50 outline-none focus:border-indigo-500" />
        </div>
      </div>
