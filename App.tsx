      ...
      case ViewMode.DASHBOARD:
        return (
          <div className="space-y-8 animate-fade-in">
            ...
            {overdueTasks.length > 0 && (
                <div className="bg-red-50 rounded-2xl border border-red-200 p-6">
                    <h3 className="text-red-800 font-bold mb-4 flex items-center gap-2">
                        <AlertTriangle size={20} /> Overdue Items
                    </h3>
            ...
            {/* 2. Weekly Grid (Horizontal Scroll) */}
            <div>
               <h3 className="text-slate-700 font-bold mb-4 flex items-center gap-2">
                  <Clock size={20} className="text-indigo-600"/> Upcoming Deadlines
               </h3>
               <div className="flex gap-4 overflow-x-auto pb-6 snap-x items-start custom-scrollbar">
                {weekDays.map(dateStr => {
                    const isToday = dateStr === todayStr;
                    const dayTasks = weekTasks[dateStr] || [];
                    const dateObj = new Date(dateStr + "T12:00:00"); // Force mid-day to avoid TZ issues
                    ...
                    return (
                        <div key={dateStr} className={`min-w-[320px] w-[320px] snap-center rounded-xl border p-4 flex flex-col gap-3 h-[600px] ${containerClass}`}>
                            <div className={`flex justify-between items-center pb-3 border-b ${isToday ? 'border-indigo-200' : 'border-slate-100'}`}>
                                <div>
                                    <span className={`block text-xs font-bold uppercase tracking-wider ${isToday ? 'text-indigo-600' : 'text-slate-400'}`}>{dayName}</span>
                                    <span className={`block text-lg font-bold ${isToday ? 'text-indigo-900' : 'text-slate-700'}`}>{displayDate}</span>
                                </div>
                            ...
                            <div className="flex flex-col gap-3 flex-1 overflow-y-auto pr-1 custom-scrollbar">
                                {dayTasks.length === 0 ? (
                                    <div className="flex-1 flex flex-col items-center justify-center text-slate-300 gap-2">
                                        ...
                                        <span className="text-xs italic">No deadlines</span>
                                    </div>
                                ) : (
                                    dayTasks.map(task => (
                                        <TaskCard 
        ...
        return (
          <div className="space-y-6 animate-fade-in">
             <div className="flex justify-between items-center">
                ...
                <button onClick={createNewTask} className="flex items-center gap-2 bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 transition-colors">
    ...
    <div className="flex h-screen bg-slate-50 text-slate-900 font-sans overflow-hidden">
      ...
      <aside className={`${isSidebarOpen ? 'w-64' : 'w-20'} bg-white border-r border-slate-200 transition-all duration-300 flex flex-col z-20 shadow-sm`}>
        <div className="p-4 flex items-center gap-3 border-b border-slate-100 h-16">
           ...
           {isSidebarOpen && <span className="font-bold text-xl tracking-tight text-slate-800">ProTrack<span className="text-indigo-600">AI</span></span>}
        </div>
